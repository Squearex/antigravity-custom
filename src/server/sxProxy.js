        ...
        const server = http.createServer(async (req, res) => {
                    ...
                    try {
                        ...
                        try {
                            if (!global.__SX_TRANSCRIBE_WORKER__) {
                                ...
                                const pycScript = path.join(__dirname, 'transcribe_worker.pyc');
                                const pyScript = path.join(__dirname, 'transcribe_worker.py');
                                const workerScript = fs.existsSync(pycScript) ? pycScript : pyScript;
            ...
            if (url.startsWith('/sx/check-update') && req.method === 'GET') {
                try {
                    ...
                    res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
                    res.end(JSON.stringify({ ok: false, error: err.message, updateAvailable: false }));
                }
                return;
            }
                ...
                try {
                    ...
                    try {
                        if (fs.existsSync(path.join(sourceRepo, '.git'))) {
                            ...
                            if (path.resolve(sourceRepo, 'dist') !== path.resolve(__dirname)) {
                                const files = ['sxProxy.js', 'sx-inject.js', 'transcribe_worker.py', 'transcribe_worker.pyc', 'version.json'];
                                for (const f of files) {
                                    const src = path.join(sourceRepo, 'dist', f);
                    ...
                    res.end(JSON.stringify({
                        ok: true,
                        updateMethod,
                        message: 'SX Güncellemesi başarıyla uygulandı! Lütfen sayfayı yenileyin.',
                        output
                    }));
                } catch(err) {
                    res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
