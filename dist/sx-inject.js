(function() {
            ...
            if (typeof F$ !== 'undefined') {
                ['dark', 'light'].forEach(mode => {
                    if (F$[mode] && !F$[mode].__sxPatched) {
                        ...
                        Object.defineProperty(F$[mode], '__sxPatched', {
                            value: true,
                            enumerable: false,
                            configurable: true,
                            writable: true
    ...
    // ────────────────────────────────────────────────────────────────────────
    // Settings > Appearance Tab — Pure Native Integration
    // ────────────────────────────────────────────────────────────────────────
    function trySXAppearanceSettingsInject() {
        // Clean up any legacy injected custom elements
        document.getElementById('sx-theme-studio')?.remove();
        document.getElementById('sx-preset-combobox-btn')?.remove();
        document.getElementById('sx-quick-presets-bar')?.remove();
        document.getElementById('sx-preset-dropdown-popover')?.remove();

        const dialog = document.querySelector('[role="dialog"]');
        if (!dialog) return;

        // Ensure native combobox and listbox are fully unhidden and styled natively
