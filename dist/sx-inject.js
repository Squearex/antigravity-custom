  ...
  var VoiceRecorder = class {
    ...
    async stopRecording(btn = null) {
      ...
      if (targetBtn) {
        ...
        targetBtn.title = "Ses \xE7\xF6z\xFCmleniyor...";
      }
  ...
  var UIInjector = class {
      ...
      document.addEventListener("click", (e) => {
        ...
        if (inDialog) {
          ...
          if (clickedItem) {
            ...
            const knownTabs = ["general", "genel", "application", "uygulama", "appearance", "g\xF6r\xFCn\xFCm", "customization", "\xF6zelle\u015Ftirme", "browser", "taray\u0131c\u0131", "conversation", "sohbet", "shortcut", "k\u0131sayol", "feedback", "models"];
    ...
    getActiveSettingsTab(dialog, sidebar) {
      ...
      if (sidebar) {
        const allItems = Array.from(sidebar.querySelectorAll("*")).filter((el) => {
          ...
          return ["general", "genel", "application", "uygulama", "appearance", "g\xF6r\xFCn\xFCm", "models", "customizations", "browser", "conversations", "shortcuts", "feedback"].some((k) => txt === k || txt.startsWith(k));
        });
    ...
    hookDOM() {
      ...
      let actionContainer = document.querySelector(
        'div.flex.items-center.gap-1:has([data-tooltip-id*="input-send-button"]), div.flex.items-center.gap-1:has([data-testid="send-button"]), div.flex.items-center.gap-1:has([data-testid="stop-button"]), div.flex.items-center.gap-1:has(button[aria-label*="Record voice" i]), div.flex.items-center.gap-1:has(button[aria-label*="Stop" i])'
      );
    ...
    _dockModelPanelAboveChat(targetBox, modelPanel) {
      ...
      const card = document.querySelector('[contenteditable="true"], textarea')?.closest('[class*="rounded-2xl"], form') || document.getElementById("antigravity.agentSidePanelInputBox") || document.querySelector(".bg-card-border");
      if (!card) return;
